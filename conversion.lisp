...
(defun hex<- (data)
  (cond ((typep data 'integer)
        ...
        ((or (typep data '(vector (unsigned-byte 8)))
         ...
         (octets->hex data))
        ((typep data 'string)
         (error (format nil "'~A' is already a string." data)))
...
(defun octets<- (data)
  (cond ((typep data 'integer)
        ...
        ((or (typep data '(vector (unsigned-byte 8)))
             (typep data '(simple-array (unsigned-byte 8) (*))))
         (error (format nil "'~S' is already an octet-vector." data)))
        ((typep data 'string)
         (hex->octets data))
        (t (error "Type of value N not recognized."))))
...
(defun int<- (data)
  (cond ((typep data 'integer)
         (error (format nil "'~D' is already an integer." data)))
        ((typep data 'bit-vector)
         (bits->int data))
        ((or (typep data '(vector (unsigned-byte 8)))
             (typep data '(simple-array (unsigned-byte 8) (*))))
...
(defun bits<- (data)
  (cond ((typep data 'integer)
         (int->bits data))
        ((typep data 'bit-vector)
         (error (format nil "'~S' is already a bit-vector" data)))
        ((or (typep data '(vector (unsigned-byte 8)))
             (typep data '(simple-array (unsigned-byte 8) (*))))
         (octets->bits data))
        ((typep data 'string)
